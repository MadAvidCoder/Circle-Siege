...
use crate::{BandDetect, BAND_EMA, HOP, ONSET_DEV_EMA, ONSET_MEAN_EMA, WINDOW};
...
impl ProcessorState {
    ...
    pub fn process(&mut self, samples: &[f32]) -> Vec<Record> {
        ...
        records.push(Record::Energy(EnergyRecord { t: self.time, e: self.e_smooth }));
        ...
        let power_spectrum: Vec<f32> = self.fft_in[0..=WINDOW / 2].iter().map(|c| c.norm_sqr()).collect();

        records.push(
            Record::Spectrum(
                SpectrumRecord { t: self.time, bins: power_spectrum.clone() })
        );
        ...
        for b in 0..3 {
            ...
            if onset > detector.mean + self.threshold * detector.dev && detector.refractory == 0 {
                let strength = ((onset - (detector.mean + self.threshold * detector.dev)) / (detector.mean + self.threshold * detector.dev + 1e-8)).clamp(0.0, 1.0);
